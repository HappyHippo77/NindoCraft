...
execute at @e[type=#du:living,tag=!du_click_entity,distance=..1] unless score @e[type=#du:living,tag=!du_click_entity,sort=nearest,limit=1] du_uuid = @s du_uuid if block ~ ~-1 ~ #naruto:loose run tag @e[type=#du:living,tag=!du_click_entity,sort=nearest,limit=1] add hh.beachcrabbed
