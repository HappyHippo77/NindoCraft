...
execute as @s at @s run execute as @e[type=#du:living,tag=!du_click_entity,distance=..2,sort=nearest,limit=1] unless score @s du_uuid = @e[tag=hh.fireballstand,sort=nearest,limit=1] du_uuid store result entity @s Motion[2] double 0.000010 run scoreboard players get @s hh.posZ